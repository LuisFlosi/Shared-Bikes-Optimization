...
dataPath = "/Users/Douzi/Desktop/"
arriving = read.csv(paste(dataPath,"arriving_structuredaaa.csv",sep = "/"))
...
mean_midnight = read.csv(paste(dataPath,"mean_bikes_at_midnight.csv",sep="/"))
...
head(mean_midnight)
prev_rslt = readRDS(paste(dataPath,"result.rds",sep="/"))
...
#shp_full = shp_full[is.na(shp_full$bikes_before_optm)][, bikes_before_optm := shift(bikes_after_optm,4, type="lead") - shift(predicted_leav,4, type="lead") + shift(predicted_arr,4, type="lead")]
library(tidyverse)
shp_full = shp_full %>%
  group_by(station_id) %>%
...
write.csv(shp_full, paste(dataPath,"station_hourly_supply_demand_200_station.csv",sep="/"))
...
write.csv(dat, paste(dataPath,"divvy.csv",sep="/"))
