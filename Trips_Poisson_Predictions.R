...
arr_pred = glm(arriving_trips ~ station_id*end_hour + tempC + days_of_week, data = train_arr, family = "poisson")
summary(arr_pred)
arr_pred$deviance / arr_pred$df.residual # no over dispersion
pchisq(arr_pred$deviance, arr_pred$df.residual, lower.tail=F) # shows good fit

leav_pred = glm(leaving_trips ~ (station_id + start_hour)^2 + tempC + days_of_week, data = train_leav, family = "poisson")
summary(leav_pred)
leav_pred$deviance / leav_pred$df.residual # no over dispersion
pchisq(leav_pred$deviance, leav_pred$df.residual, lower.tail=F) # shows good fit
